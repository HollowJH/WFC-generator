...
-   **Optimization (Garbage Collection & Memory Layout)**: To prevent Garbage Collector (GC) pauses during intense generation, we refactored core data structures to minimize object allocation.
    -   **Structure of Arrays (SoA)**: The Min-Heap uses `Int32Array` and `Float64Array` instead of allocating node objects.
    -   **Flat Stacks**: Propagation and history stacks use flattened arrays to avoid creating thousands of ephemeral objects per step.
    -   **In-Place Operations**: Array filtering was replaced with swap-and-pop O(1) removal to prevent array reallocation.
-   **Optimization (Heuristic Tie-Breaking)**: To improve generation quality and reduce backtracking, we implemented a "Degree Heuristic". When multiple cells have the same lowest entropy, the solver prioritizes the cell with the most uncollapsed neighbors (the most constraining variable). This reduces the likelihood of creating isolated, unresolvable pockets.
...
-   **Performance (Chunked Execution)**: The "Skip Animation" mode uses a time-budgeted loop (100ms chunks) in the Web Worker. Instead of blocking the thread until completion, it yields periodically to send progress updates, ensuring the UI remains responsive and the user receives feedback (remaining cell count) even during heavy calculations.
